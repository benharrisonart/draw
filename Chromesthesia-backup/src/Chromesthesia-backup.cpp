...
Encoder encoderB(D11,D12);
Encoder encoderC(D13,D14);
//PHOTON
...
//ENCODER BUTTON
const int BTNA = D6;//encoder button 
Button buttEncA(6);
...
void setup() {
    pinMode(19,OUTPUT);//speaker 1
    pinMode(18,OUTPUT);//speaker 2
    pinMode(17,OUTPUT);//speaker 3
...
//ENCODER CODE
      ...
      if (prevEncA != vEncA){Serial.printf ("Encoder A position: %i\n",vEncA);
            ...
            display.display();
            display.setCursor(0,0);
            display.printf("Note 1: %c",arrNote[vAdjA]);
            //display.printf("X1: %c%i",arrNote[vAdjA],vFreqA);
            display.display();
            pixel.setPixelColor(0,rainbow[vAdjA]);
            //pixel.setPixelColor(3,rainbow[vAdjA]);//original test pixel
            for(i=0;i<20;i++){pixel.setPixelColor(i,rainbow[vAdjA]);}pixel.show();
            //pixel.setPixelColor(4,rainbow[vAdjA]);
            pixel.show();
            noTone(19);
            tone(19,vFreqA);
      }
      if (prevEncB != vEncB){Serial.printf ("Encoder B position: %i\n",vEncB);
            ...
            display.fillRect(0, 23, 128, 22, BLACK);
            display.display();
            display.setCursor(0,23);
...
// }
    ...
    //}
    // if (buttEncB.isClicked()){buttonEncStateC = !buttonEncStateC;
    //         if (buttonEncStateC==TRUE){tone(8,vFreqC); }else{noTone(8);}
    // }
